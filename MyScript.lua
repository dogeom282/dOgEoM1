...
local function setOwnerKickLoop()
    ...
    while SetOwnerKickT do
        pcall(function()
            ...
            if not myHRP or not targetHRP or not cam then
                currentIndex = currentIndex + 1
                wait018()
                return
            end
    ...
    Callback = function(v)
        ...
        if v then
            ...
            if not SetNetworkOwner or not DestroyGrabLine then
                Rayfield:Notify({ Title = "❌ 리모트 없음", Duration = 2 })
                setOwnerToggle:Set(false)
                return
            end
            if setOwnerThread then
                task.cancel(setOwnerThread)
            end
...
local setOwnerModeLabel = SetOwnerKickTab:CreateLabel("모드: Up", 4483362458)
local setOwnerCountLabel = SetOwnerKickTab:CreateLabel("타겟: 0", 4483362458)
local setOwnerTotalLabel = SetOwnerKickTab:CreateLabel("총 호출: 0", 4483362458)
...
spawn(function()
    ...
    while true do
        if SetOwnerKickT then
            ...
            setOwnerTotalLabel:Set("총 호출: " .. setOwnerTotalCalls)
            
            local now = tick()
            if now - lastTime >= 1 then
                local speed = setOwnerTotalCalls - lastCalls
                setOwnerModeLabel:Set("속도: " .. speed .. "회/초")
                lastCalls = setOwnerTotalCalls
                lastTime = now
            end
        else
            setOwnerStatus:Set("상태: ⚫ 대기")
            setOwnerModeLabel:Set("모드: " .. setOwnerMode)
        end
