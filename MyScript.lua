...
-- [ 집 텔레포트 함수 ]
-- =============================================
local function teleportToHouse(name, pos)
    local char = plr.Character
    if not char then 
        Rayfield:Notify({Title = "오류", Content = "캐릭터 없음", Duration = 2})
        return 
...
-- 2열로 정렬하기 위해 버튼 2개씩 묶음
for i = 1, #houses, 2 do
    local rowSection = HouseTeleportTab:CreateSection("")
    
    -- 첫 번째 버튼
    rowSection:CreateButton({
        Name = houses[i][1],
        Callback = function()
            teleportToHouse(houses[i][1], houses[i][2])
        end
    })
    ...
    if houses[i+1] then
        rowSection:CreateButton({
            Name = houses[i+1][1],
            Callback = function()
                teleportToHouse(houses[i+1][1], houses[i+1][2])
            end
        })
    end
end
...
HouseTeleportTab:CreateButton({
    Name = "📍 스폰 (0, -7, 0)",
    Callback = function()
        teleportToHouse("스폰", Vector3.new(0, -7, 0))
    end
})
