-- FTAP (Fling Things and People) 올인원 스크립트 (안티 페인트 추가)
        ...
        while AntiBurnV do
            ...
            if char then
                ...
                if head then
                    ...
                    -- 위치는 머리 근처로 (시야 방해 최소화)
                    EP.CFrame = head.CFrame * CFrame.new(0, 10, 0)  -- 머리 위 10스터드
                end
            end
            task.wait(0.1)  -- 체크 주기 줄임
        end
...
local AntiBurnToggle = SecurityTab:CreateToggle({
    Name = "🔥 Anti-Burn (시야방지 개선)",
    CurrentValue = false,
    Callback = function(Value)
        AntiBurnV = Value
        AntiBurn()
        Rayfield:Notify({Title = "안티 불", Content = Value and "활성화 (머리 위로 이동)" or "비활성화", Duration = 2})
    end
})
...
Rayfield:Notify({
    Title = "🚀 로드 완료",
    Content = "안티 불 시야방지 개선 + 안티 페인트 추가",
    Duration = 5
})    button.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch then
            dragging = true
